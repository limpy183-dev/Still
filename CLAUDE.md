...
| Android app (preview) | `android/` | Plain Java, Android framework only (no AndroidX/libraries). An accessibility service blocks apps; `Session.java` mirrors `native/Session.cs` and is JVM unit-tested. Build/test: `cd android && ./gradlew testDebugUnitTest assembleDebug`. See `docs/android.md`. Not released or on the website yet. |
