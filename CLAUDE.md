...
| Android app (preview) | `android/` | Plain Java, Android framework only (no AndroidX/libraries). An accessibility service blocks apps and (via browser address bars) websites, and enforces daily website limits and bedtime; `Session.java` mirrors `native/Session.cs`, `Websites.java` mirrors `app/websites.js`; both are JVM unit-tested. Build/test: `cd android && ./gradlew testDebugUnitTest assembleDebug`. See `docs/android.md`. Not released or on the website yet. |
